...
if(isset($_POST['uname']) && isset($_POST['name']) && 
   isset($_POST['password']) && isset($_POST['re_password'])){
    ...
    if(empty($uname)){
        ...
        header("Location:sign_up?error=Name is required.&$user_data");
        exit();
    }else if(empty($pass)){
        header("Location:sign_up?error=Password is required.&$user_data");
        exit();
    }else if(empty($re_pass)){
        header("Location:sign_up?error=Re Password is required.&$user_data");
        exit();
    }else if($pass !== $re_pass){
        header("Location:sign_up?error=The confirmation password does not match.&$user_data");
        exit();
    }else {
